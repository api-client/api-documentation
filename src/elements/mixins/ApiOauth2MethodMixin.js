...
/** @typedef {import('@api-client/amf-store').ApiDomainExtension} ApiDomainExtension */
/** @typedef {import('@api-client/amf-store').ApiDataNodeUnion} ApiDataNodeUnion */
...
export const findOauth2CustomSettings = Symbol('findOauth2CustomSettings');
export const computeGrants = Symbol('computeGrants');
...
const mxFunction = (base) => {
  class ApiOauth2MethodMixinImpl extends AmfParameterMixin(base) {
    get grantType() {
    ...
    [initializeOauth2Model]() {
      ...
      if (!info.scheme || !info.scheme.type || info.scheme.type !== 'OAuth 2.0') {
        this[setupOAuthDeliveryMethod]();
        this[updateGrantTypes]();
        return;
      }
      
      this[setupOAuthDeliveryMethod](info.scheme);
    ...
    /**
     * @param {ApiSecuritySchemeRecursive=} scheme
     */
     ...
     * not specifies.
     *
     * @param {ApiSecuritySchemeRecursive} info Security AMF model
    ...
    [preFillAmfData](model){
      ...
      if (grants.length) {
        const index = authorizationGrants.indexOf('code');
        if (index !== -1) {
          authorizationGrants[index] = 'authorization_code';
        }
        this[updateGrantTypes](authorizationGrants);
      } else {
        this[updateGrantTypes]();
      }
     ...
     * @param {ApiSecurityOAuth2Settings} model AMF model describing settings of the security
     * scheme
     * @returns {ApiDomainExtension|null} The extension definition or null
     */
    [findOauth2CustomSettings](model) {
      ...
      return customDomainProperties.find((property) => {
        if (!property.extension || !property.extension.types.includes(ns.aml.vocabularies.data.Object)) {
          return false;
        }
        const typed = /** @type ApiObjectNode */ (property.extension);
        return Object.keys(typed.properties).some(name => properties.includes(name));
      });
    }
    ...
    [computeGrants](grans=[], extension) {
      ...
      const typed = /** @type ApiObjectNode */ (extension.extension);
    ...
    [setupAnnotationParameters](annotation) {
      ...
      /* istanbul ignore if */
      if (!annotation || !annotation.extension) {
        return;
      }
      const typed = /** @type ApiObjectNode */ (annotation.extension);
    ...
    [computeCustomParameters](params, reportKey) {
      ...
      const report = AmfInputParser.reportRequestInputs(params.map(p => p.parameter), InputCache.getStore(this, this.globalCache), this[nilValues]);
